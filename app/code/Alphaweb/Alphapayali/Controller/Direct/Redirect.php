...
namespace Alphaweb\Alphapayali\Controller\Direct;

use Magento\Framework\App\Action\Action;
use Alphaweb\Alphapayali\Model\Alphapayali as AlphapayaliCode;
...
{
    /**
     * @var \Alphaweb\Alphapayali\Model\Api\RedirectApi
     */
   // protected $_api;
   ...
   // protected $_detect;
    ...
    public static function get_alphapay_baseurl($_paymentInstance){
        $currency_urls = [
            'CAD' => 'https://pay.alphapay.ca',
            'USD' => 'https://cashier.alphapay.com'
        ];
        $platform = $_paymentInstance->getConfigData('platform');
        if(empty($currency_urls[$platform])) $platform = 'CAD';
        return $currency_urls[$platform];
    }
    ...
    public function execute()
    {
        ...
        if($this->is_wechat_client()){
            ...
            <html>
                ...
                <?php
                	if($this->isIOS()){
                		?>
                		<img alt="支付宝" src="<?php echo $this->assetRepo->getUrl('Alphaweb_Alphapayaliali::images/ios.png')?>" style="max-width: 100%;">
                		<?php
                	}else{
                		?>
                		<img alt="支付宝" src="<?php echo $this->assetRepo->getUrl('Alphaweb_Alphapayaliali::images/alipayout.jpg')?>" style="max-width: 100%;">
                		<?php
                	}
                ?>
                </body>
        ...
        try {
            ...
            if ($this->_quote->getId()) {
                ...
                if (!$this->_quote->getGrandTotal()) {
                    throw new \Magento\Framework\Exception\LocalizedException(
                        __(
                            'Alphapayali can\'t process orders with a zero balance due. '
            ...
            $channel=null;
            $base_url = self::get_alphapay_baseurl($paymenthod);
            if($this->is_app_client()){
                $channel='Alipay';
                $api_uri ="$base_url/api/v1.0/h5_payment/partners/%s/orders/%s";
            }else{
                $channel='Alipay';
                $api_uri ="$base_url/api/v1.0/web_gateway/partners/%s/orders/%s";
            }
